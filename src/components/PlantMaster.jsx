...
const PlantMaster = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this plant?')) {
      await supabase.from('plant_master').delete().eq('id', id);
      fetchPlants();
    }
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4 mb-6">
        <input name="plant_id" placeholder="Plant ID" value={form.plant_id} onChange={handleChange} className="p-2 border rounded" required />
        <input name="description" placeholder="Description" value={form.description} onChange={handleChange} className="p-2 border rounded" required />
        <select name="status" value={form.status} onChange={handleChange} className="p-2 border rounded">
          <option>Active</option>
          <option>Inactive</option>
        </select>
        <input name="tax_reg_no" placeholder="Tax Reg. No." value={form.tax_reg_no} onChange={handleChange} className="p-2 border rounded" />
        <input name="license" placeholder="License" value={form.license} onChange={handleChange} className="p-2 border rounded" />
        <input name="gs1_prefix" placeholder="GS1 Prefix" value={form.gs1_prefix} onChange={handleChange} className="p-2 border rounded" />
        <input name="address1" placeholder="Address" value={form.address1} onChange={handleChange} className="p-2 border rounded" />
